...
def daily_kg_to_annual_tonnes(kg):

    Annual_kg = daily_to_annual(kg)

    Annual_tonnes = kg_to_tonnes(Annual_kg)

    return Annual_tonnes
#this is going to be used for diet
#but to be honest I really think that helper functions for this whole page
#is not necessary, I am creating it just to be able to get full mark(according
#to the instruction).
...
def fp_of_computing(daily_online_use, daily_phone_use, new_light_devices, new_medium_devices, new_heavy_devices):
    ...
    3.7304
    '''
    #fist find out the annual footprint for each objects then add them together

    Online = daily_to_annual(0.000001 * (daily_online_use * 55)) 
    #0.000001 is the conversion for g to tonnes

    Phone = daily_phone_use * 1250  #numbers are all from the source above

    Small_device = new_light_devices * 75

    Laptop = new_medium_devices * 200

    Work_station = new_heavy_devices * 800
    
   
    return Online + kg_to_tonnes(Phone + Small_device + Laptop + Work_station)
...
def fp_of_diet(daily_g_meat, daily_g_cheese, daily_L_milk, daily_num_eggs):
    ...
    Meat = daily_g_meat * 0.0268
    
    Egg = daily_num_eggs * 0.3

    Milk = daily_L_milk * 0.2677777

    Cheese = daily_g_cheese * 0.012

    #all numbers are from the source above (I didn't make up
    #any value, please don't throw me into jail Mr.Officer QAQ)

    return daily_kg_to_annual_tonnes(Vegan + Meat  + Cheese  + Milk + Egg) 
                                        
    #I did the conversion in the end so that I dont have to do it four times
    # and also I have used helper function so may I get full mark (・ω・)? 
#################################################
